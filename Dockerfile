...
COPY ["ThumbnailGenerator/ThumbnailGenerator.csproj", "ThumbnailGenerator/"]
RUN dotnet restore "ThumbnailGenerator/ThumbnailGenerator.csproj"

# Copy source còn lại và build
WORKDIR "/src/ThumbnailGenerator"
COPY ThumbnailGenerator/ .
RUN dotnet build "ThumbnailGenerator.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Publish
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "ThumbnailGenerator.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
